...
Outputs (frontend/):
    electricity-gdp.html                       the rendered page (data baked in)
    files/electricity-gdp-panel-2000-2023.csv  the merged country-year panel
"""
import json
import csv
import math
...
OUT_HTML = FRONTEND / "electricity-gdp.html"
OUT_CSV = FRONTEND / "files" / "electricity-gdp-panel-2000-2023.csv"
...
# --------------------------------------------------------------- data & stats
def load_panel():
    """Return list of (iso, name, year, gdp, elec) with both values present."""
    elec = {}
    with open(ELEC_CSV) as f:
        for row in csv.DictReader(f):
            ...
            if c and len(c) == 3:
                try:
                    elec[(c, int(row["year"]))] = float(row["per_capita_total_demand__kwh"])
                except ValueError:
                    pass
    panel = []
...
def analyse(panel):
    ...
    below = sum(1 for p, _ in hi if p[4] < 2000)
...
def build_density(panel, st):
    ...
    s = [f'<svg viewBox="0 0 {VW} {VH}" role="img" aria-label="Density of {st["n"]} '
         f'country-year observations in the log GDP per capita by log electricity per '
    ...
    for e, lab in [(100, '100'), (1000, '1,000'), (10000, '10,000')]:
        y = sy(math.log10(e))
    ...
    # empty-corner outline: G >= $30k and E < 2000 kWh
    ex0, ey0 = sx(math.log10(30000)), sy(math.log10(2000))
    ...
    mid = (PY0 + PY1) / 2
    s.append(f'<text class="axtitle" x="22" y="{mid:.1f}" text-anchor="middle" transform="rotate(-90 22 {mid:.1f})">Electricity use per capita (kWh/yr, log scale)</text>')
...
<head>
    ...
    <title>Jonathan Keogh | No high-GDP, low-electricity country exists</title>
    ...
    <meta name="description" content="Across the full 2000-2023 panel of 4,700+ country-years, the joint law of GDP per capita and electricity use per capita puts no mass in the high-income, low-consumption corner." />
    <script>
      MathJax = {{
...
<section>
<p>Take every country with both figures reported, once per year from {y0} to {y1}: a panel of <strong>{n:,} country-year observations</strong> of GDP per capita $G$ and electricity use per capita $E$. The title is a statement about the <em>support</em> of their joint law&mdash;the region $\\{{G \\text{{ large}},\\, E \\text{{ small}}\\}}$ carries no mass&mdash;and it survives a close look.</p>
</section>
...
<li><strong>Within</strong> a country over time (deviations): $r_W = {rw:.2f}$, slope $\\beta_W = {bw:.2f}$. When a country&rsquo;s own income rises, its electricity rises too, but sub-proportionally&mdash;generating capital is slow to build and rich economies tilt toward services.</li>
</ul>
...
<p>The claim concerns the lower edge of the cloud, not its center. Binning by income and taking the 5th percentile of $E$ (the green wall) yields a lower envelope that is monotone increasing: lift the income floor and the electricity floor lifts with it. Sharpening to the corner, of the <strong>{hi_n} country-years</strong> with $G \\ge \\${thr30}{{,}}000$, none has $E < {thr2}$&nbsp;kWh&mdash;the lowest is {hi_min_e:,.0f}&nbsp;kWh ({hi_min_name}, {hi_min_year}). Empirically</p>
...
<section>
<h2>Two caveats</h2>
<p>First, country-years are not independent: a country contributes up to {nyears} near-repeated points, so the effective sample is far below {n:,}. The honest content is <em>persistence across {nyears} annual cross-sections</em>, not {n:,} independent draws. Second, correlation fixes no direction&mdash;cheap power enables output and output funds power, with common drivers (industrialisation, institutions) behind both.<label for="sn-1" class="margin-toggle sidenote-number"></label><input type="checkbox" id="sn-1" class="margin-toggle"><span class="sidenote">The literature calls this the &ldquo;electricity&ndash;growth nexus&rdquo;; the direction of causation genuinely varies by country.</span> Neither caveat touches the support statement, which is what the title asserts: no country has yet been wealthy without consuming a great deal of electricity.</p>
</section>

                <footer class="de-emphasize">
                    <p>Sources: Our World in Data (Ember; Energy Institute Statistical Review of World Energy) &amp; World Bank, {y0}&ndash;{y1}. <a href="files/electricity-gdp-panel-2000-2023.csv">Download the panel (CSV)</a>.</p>
                </footer>
            </article>
...
def main():
    ...
    with open(OUT_CSV, "w", newline="") as f:
        w = csv.writer(f)
        w.writerow(["iso3", "country", "year", "gdp_per_capita_usd", "electricity_per_capita_kwh"])
    ...
    print(f"empty corner: {st['below']} of {st['hi_n']} high-income country-years below 2000 kWh")
